...
namespace ETMS.DataAccess.Alien.Lib
{
    public class AlienPermissionData
    {
            ...
            get
            {
                if (_menuConfigs == null)
                {
                    lock (LockMenuObj)
                    {
                        if (_menuConfigs == null)
                        {
                            var json = File.ReadAllText(FileHelper.GetFilePath("alienmenusetting.json"));
            ...
            get
            {
                if (_routeConfigs == null)
                {
                    lock (LockRouteConfig)
                    {
                        if (_routeConfigs == null)
                        {
                            var json = File.ReadAllText(FileHelper.GetFilePath("alienroutesetting.json"));
