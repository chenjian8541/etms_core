...
namespace ETMS.DataAccess
{
    public class TenantToDoThingDAL : DataAccessBase<TenantToDoThingBucket>, ITenantToDoThingDAL
    {
        ...
        {
            var sql = $"SELECT COUNT(0) FROM EtClassTimes WHERE TenantId = {_tenantId} AND IsDeleted = {EmIsDeleted.Normal} AND [Status] = {EmClassTimesStatus.UnRollcall} AND ClassOt <= '{DateTime.Now.EtmsToDateString()}'";
            var obj = await _dbWrapper.ExecuteScalar(sql);
            return obj.ToInt();
        }
