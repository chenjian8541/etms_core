...
namespace ETMS.DataAccess
{
    public class TempStudentNeedCheckDAL : DataAccessBase<TempStudentNeedCheckCountBucket>, ITempStudentNeedCheckDAL
    {
        ...
        private async Task<bool> TempStudentNeedCheckClassDelOldHis()
        {
            ...
            {
                LOG.Log.Fatal("TempStudentNeedCheckClassDelOldHis", ex1, this.GetType());
                try
                {
                    await _dbWrapper.Execute($"UPDATE EtTempStudentNeedCheckClass SET IsDeleted = {EmIsDeleted.Deleted} WHERE TenantId = {_tenantId} AND Ot <= '{hisOt.EtmsToDateString()}' ");
                    return true;
                }
                catch (Exception ex2)
                {
                    LOG.Log.Error("TempStudentNeedCheckClassDelOldHis", ex2, this.GetType());
                    return true;
                }
            }
        }
