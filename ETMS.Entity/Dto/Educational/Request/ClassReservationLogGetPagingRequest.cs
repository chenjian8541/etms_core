...
{
    public class ClassReservationLogGetPagingRequest : RequestPagingBase
    {
        ...
        public override string ToString()
        {
            ...
            if (Status != null)
            {
                if (Status == EmClassTimesReservationLogStatus.Invalidation)
                {
                    condition.Append($" AND ([Status] = {Status.Value} OR ([Status] = {EmClassTimesReservationLogStatus.Normal} AND ClassOt < '{DateTime.Now.EtmsToDateString()}'))");
                }
                else if (Status == EmClassTimesReservationLogStatus.Normal)
                {
                    condition.Append($" AND [Status] = {Status.Value} AND ClassOt >= '{DateTime.Now.EtmsToDateString()}'");
                }
                else
                {
