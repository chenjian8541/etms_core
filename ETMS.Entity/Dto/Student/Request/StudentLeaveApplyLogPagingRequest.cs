...
{
    public class StudentLeaveApplyLogPagingRequest : RequestPagingBase
    {
        ...
        public override string ToString()
        {
            ...
            if (StartOt != null)
            {
                condition.Append($" AND ApplyOt >= '{StartOt.Value.EtmsToDateString()}'");
            }
            if (EndOt != null)
            {
                condition.Append($" AND ApplyOt < '{EndOt.Value.EtmsToDateString()}'");
            }
            if (HandleStatus != null)
            {
                condition.Append($" AND [HandleStatus] = {HandleStatus.Value}");
            }
            condition.Append($" AND [HandleStatus] <> {EmStudentLeaveApplyHandleStatus.IsRevoke}");
