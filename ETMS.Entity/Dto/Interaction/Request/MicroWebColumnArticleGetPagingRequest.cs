...
{
    public class MicroWebColumnArticleGetPagingRequest : RequestPagingBase
    {
        ...
        public override string ToString()
        {
            ...
            if (StartOt != null)
            {
                condition.Append($" AND UpdateTime >= '{StartOt.Value.EtmsToDateString()}'");
            }
            if (EndOt != null)
            {
                condition.Append($" AND UpdateTime < '{EndOt.Value.EtmsToDateString()}'");
            }
            if (!string.IsNullOrEmpty(ArTitile))
            {
                condition.Append($" AND ArTitile LIKE '%{ArTitile}%'");
            }
