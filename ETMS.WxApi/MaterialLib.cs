...
namespace WxApi
{
    ...
    public class MaterialLib
    {
        ...
        /// <returns></returns>
        public static UpLoadInfo AddImage(string accessToken, byte[] b, string name, string fileName)
        {
            try
            {
                //永久素材接口
                string url = "http://api.weixin.qq.com/cgi-bin/material/add_material?access_token={0}&type={1}";
                return Utils.PostResult<UpLoadInfo>(string.Format(url, accessToken, "image"), b, name, fileName);
            }
            catch (Exception e)
            {
                return new UpLoadInfo
