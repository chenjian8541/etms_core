...
{
    public abstract class BaseTenantALLHandle : BaseJob
    {
        protected ISysTenantDAL _sysTenantDAL;

        private const int _pageSizeTenant = 100;

        public BaseTenantALLHandle(ISysTenantDAL sysTenantDAL)
        {
        ...
        {
            var pageCurrent = 1;
            var getTenantsEffectiveResult = await _sysTenantDAL.GetAllTenant(_pageSizeTenant, pageCurrent);
            ...
            {
                getTenantsEffectiveResult = await _sysTenantDAL.GetAllTenant(_pageSizeTenant, pageCurrent);
                await HandleTenantList(getTenantsEffectiveResult.Item1);
                pageCurrent++;
            }
        }
